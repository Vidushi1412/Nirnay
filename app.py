...
[data-testid="stSidebar"] { background-color: #0c1018 !important; border-right: 1px solid rgba(255,255,255,0.06); }
...
with st.sidebar:
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px">
      <div style="width:36px;height:36px;border-radius:9px;background:linear-gradient(135deg,#4f6ef7,#7c3aed);
           display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:900;color:#fff">न</div>
      <div>
        <div style="font-size:17px;font-weight:800;color:#dde3f0;letter-spacing:-0.3px">निर्णय</div>
        <div style="font-size:9px;color:#3d4f68;letter-spacing:2px;text-transform:uppercase">Decision Intelligence</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.divider()
    st.markdown('<p style="color:#3d4f68;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Active Dataset</p>', unsafe_allow_html=True)

    ds_labels = {k: f"{v['icon']}  {v['name']}" for k, v in DATASETS.items()}
    selected_ds = st.selectbox("Dataset", list(ds_labels.keys()),
